...
export default function RiderDetail() {
  ...
  const goMainPrev = useCallback(() => setMainIndex(i => (i - 1 + totalMain) % totalMain), [totalMain]);
  const goMainNext = useCallback(() => setMainIndex(i => (i + 1) % totalMain), [totalMain]);

  // Gallery images = all images except the first (used as portrait)
  const galleryImages = allImages.slice(1);
  const totalModal = galleryImages.length;
  const goModalPrev = useCallback((e) => { e?.stopPropagation(); setModalIndex(i => (i - 1 + totalModal) % totalModal); }, [totalModal]);
  const goModalNext = useCallback((e) => { e?.stopPropagation(); setModalIndex(i => (i + 1) % totalModal); }, [totalModal]);
  ...
  if (error || !rider) {
    return (
      <div className="bg-[#1c1c1c] min-h-screen flex flex-col justify-center items-center text-white">
        <p className="text-red-400 tracking-widest uppercase mb-4">{error || 'Rider not found'}</p>
        <Link to="/riders" className="px-6 py-2 border border-white/50 rounded-full hover:bg-white/10 transition">
          BACK TO RIDERS
        </Link>
      </div>
    );
    ...
    <div className="bg-[#222] min-h-screen font-poppins text-white pb-24">
      ...
      <div className="relative w-full h-[40vh] sm:h-[50vh] md:h-[70vh] bg-black overflow-hidden">
        {videoUrl ? (
          ...
          <img loading="lazy" src={allImages[0].url} alt={rider.name} className="w-full h-full object-cover opacity-80" />
        ) : (
          <div className="w-full h-full bg-[#111] flex items-center justify-center text-gray-700 tracking-widest">
            NO MEDIA UPLOADED
          </div>
        )}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 -mt-16 sm:-mt-20 md:-mt-32 relative z-10">
        <div className="flex flex-col md:flex-row gap-8 md:gap-16 items-start">
          ...
          <FadeUp className="w-full md:w-1/2 shrink-0">
            {allImages.length > 0 ? (
              <div className="relative group w-full aspect-[4/5] overflow-hidden shadow-2xl bg-[#111]">
                      ...
                      {allImages.map((_, i) => (
                        <button
                          ...
                          className={`h-1.5 rounded-full transition-all duration-300 ${
                            i === mainIndex ? 'bg-white w-5' : 'bg-white/40 w-1.5'
                          }`}
                        />
                      ))}
                    </div>
                      ...
                      {allImages.map((img, i) => (
                        <button
                          ...
                          className={`w-10 h-10 rounded overflow-hidden border-2 transition-all duration-200 shrink-0 ${
                            i === mainIndex ? 'border-white scale-110' : 'border-transparent opacity-60 hover:opacity-100'
                          }`}
                        >
              ...
              <div className="w-full aspect-[4/5] bg-[#333] flex items-center justify-center text-gray-500">
                NO MAIN IMAGE
              </div>
            )}
          </FadeUp>
          ...
          <FadeUp>
            ...
            <div className="columns-1 sm:columns-2 md:columns-3 gap-4 space-y-4">
              {galleryImages.map((img, idx) => (
                <div
                  key={img.id}
                  className="break-inside-avoid relative group cursor-pointer overflow-hidden bg-[#2a2a2a]"
